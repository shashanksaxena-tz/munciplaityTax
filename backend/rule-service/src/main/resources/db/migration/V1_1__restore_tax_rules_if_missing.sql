-- V1.1: Restore tax_rules table if missing (Fix for missing relation error)
-- This ensures the table exists before V2 runs, handling cases where V1 was marked as applied but table is gone.
...
CREATE INDEX IF NOT EXISTS idx_tax_rules_temporal ON tax_rules(tenant_id, effective_date, end_date)
WHERE approval_status = 'APPROVED';

-- Rule code lookup for dependency resolution
CREATE INDEX IF NOT EXISTS idx_tax_rules_code ON tax_rules(rule_code, tenant_id, approval_status);

-- Approval workflow queries
CREATE INDEX IF NOT EXISTS idx_tax_rules_approval ON tax_rules(approval_status, created_date);

-- Version chain traversal
CREATE INDEX IF NOT EXISTS idx_tax_rules_version_chain ON tax_rules(previous_version_id)
WHERE previous_version_id IS NOT NULL;

-- Category and entity type filtering
CREATE INDEX IF NOT EXISTS idx_tax_rules_category ON tax_rules(category, tenant_id)
WHERE approval_status = 'APPROVED';
