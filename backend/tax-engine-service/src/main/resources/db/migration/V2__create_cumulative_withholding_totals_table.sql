-- Flyway Migration V2: Create cumulative_withholding_totals table
-- Feature: Withholding Reconciliation System
-- Purpose: Cached YTD cumulative totals for performance (Research R2)

CREATE TABLE IF NOT EXISTS cumulative_withholding_totals (
...
-- Comments
COMMENT ON TABLE cumulative_withholding_totals IS 'Cached YTD totals per FR-002, FR-005. Event-driven updates on W-1 filing.';
COMMENT ON COLUMN cumulative_withholding_totals.periods_filed IS 'Count of W-1 filings for tax year';
COMMENT ON COLUMN cumulative_withholding_totals.projected_annual_wages IS 'FR-005: Based on run rate (cumulative / periods * expected periods)';
COMMENT ON COLUMN cumulative_withholding_totals.on_track_indicator IS 'FR-005: Within 15% of estimated wages';
