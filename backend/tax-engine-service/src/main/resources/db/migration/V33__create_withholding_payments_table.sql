-- Flyway Migration V5: Create withholding_payments table
...
CREATE TABLE IF NOT EXISTS withholding_payments (
    ...
    CONSTRAINT fk_payment_w1_filing FOREIGN KEY (w1_filing_id) REFERENCES w1_filings(id)
);
...
CREATE INDEX idx_payment_tenant ON withholding_payments(tenant_id);

-- Comments for documentation
COMMENT ON TABLE withholding_payments IS 'Payment tracking for W-1 filings per FR-020';
COMMENT ON COLUMN withholding_payments.tenant_id IS 'Multi-tenant isolation per Constitution Principle II';
COMMENT ON COLUMN withholding_payments.transaction_id IS 'Payment gateway transaction identifier';
