    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex justify-end animate-fadeIn">
      <div className="w-full max-w-lg bg-white h-full shadow-2xl flex flex-col p-6">
        <div className="flex justify-between items-center mb-8">
           <h2 className="text-xl font-bold flex items-center gap-2"><Settings className="w-5 h-5 text-[#469fe8]"/> Business Rules</h2>
           <button onClick={onClose} className="text-[#5d6567] hover:text-[#0f1012]">Close</button>
        </div>
        ...
        <div className="space-y-8 flex-1 overflow-y-auto pr-2">
           ...
           <section className="space-y-4">
             <h3 className="text-xs font-bold uppercase text-[#5d6567] tracking-wider border-b border-[#dcdede] pb-2">General Tax Settings</h3>
           ...
           {/* Allocation Section */}
           <section className="space-y-4">
             <h3 className="text-xs font-bold uppercase text-[#5d6567] tracking-wider border-b border-[#dcdede] pb-2">Allocation (Schedule Y)</h3>
             <div className="space-y-3">
                <div>
                  <label className="text-sm font-bold text-[#102124]">Allocation Method</label>
                  <select value={rules.allocationMethod} onChange={e=>onUpdate({...rules, allocationMethod: e.target.value as any})} className="w-full border border-[#dcdede] p-2 rounded-lg mt-1 text-sm focus:ring-2 focus:ring-[#970bed] focus:border-transparent">
           ...
           {/* Schedule X Section */}
           <section className="space-y-4">
             <h3 className="text-xs font-bold uppercase text-[#5d6567] tracking-wider border-b border-[#dcdede] pb-2">Schedule X (Adjustments)</h3>
             <div className="space-y-3">
           ...
           {/* NOL Section */}
           <section className="space-y-4">
             <h3 className="text-xs font-bold uppercase text-[#5d6567] tracking-wider border-b border-[#dcdede] pb-2">Loss Carryforwards (NOL)</h3>
             <div className="flex items-center gap-2 mb-2">
               <input type="checkbox" checked={rules.enableNOL} onChange={e => onUpdate({...rules, enableNOL: e.target.checked})} className="w-4 h-4 text-[#469fe8] rounded focus:ring-2 focus:ring-[#970bed]" />
           ...
           {/* Penalties Section */}
           <section className="space-y-4 bg-[#fff5f5] p-4 rounded-xl border border-[#ec1656]/20">
             <h3 className="text-xs font-bold uppercase text-[#ec1656] tracking-wider flex items-center gap-2"><AlertTriangle className="w-3 h-3"/> Compliance & Penalties</h3>
             <div className="space-y-3">
