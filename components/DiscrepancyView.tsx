...
export const DiscrepancyView: React.FC<DiscrepancyViewProps> = ({ report, onAcceptIssue }) => {
  ...
  const handleAcceptIssue = (issue: DiscrepancyIssue) => {
  ...
  function renderIssue(issue: DiscrepancyIssue, colors: ReturnType<typeof getSeverityColor>) {
    ...
    return (
      <div key={issue.issueId} className={`${colors.bg} border ${colors.border} rounded-lg p-4 space-y-3`}>
        ...
        <div className="flex items-center gap-6 text-sm bg-white rounded-lg p-3">
          ...
          {Math.abs(issue.difference) > 0.01 && (
